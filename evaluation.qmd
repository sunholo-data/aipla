...
For each task class in the STX physics setting, identify the smallest and cheapest model that performs adequately. The result drives:

- **Production routing decisions** — which model handles each request type
- **Cost projections** — what the project will pay at scale
- **Migration timeline** — when locally-hosted open-weight models become viable for full self-hosting
...
- **Production routing** — the model abstraction layer (ADR-004 on [Architecture](architecture.qmd)) reads the capability-floor matrix to choose models per task class
- **UCPH migration timeline** — local-readiness fraction tells UCPH IT when to invest in GPU hardware (see [Self-hosting](self-hosting.qmd))
- **Strand C scoping** — new model classes (VLMs, embodied) plug into the same eval to compare against LLM baselines
- **Handover** — successors re-run quarterly; the eval is a research instrument that outlives the contract
