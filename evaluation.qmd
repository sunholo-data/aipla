...
Results against the page's **≥ 80% capability-floor threshold**, with where each model runs (tiers match the [GPQA table above](#real-numbers--gpqa-diamond-the-closest-public-proxy)):

| Model | Runs on | STX accuracy | Abstains | ≥ 80% |
|---|---|---|---|---|
| Claude sonnet / opus / haiku | Cloud API (Tier 1) | 93–96% | 5–6 | ✓ |
| gemini-3.5-flash (app top) | Cloud API — Vertex (Tier 1) | 92% | 7 | ✓ |
| **qwen3-8b** (open, 8 B) | **Small local — single 24 GB GPU, even a laptop (Tier 3)** | **88%** | 6 | ✓ |
| gemini-2.5-flash (app mid) | Cloud API — Vertex (Tier 1) | 86% | 5 | ✓ |
| deepseek-v4-flash · qwen3-next-80b-a3b (open MoE) | Server / cluster — multi-GPU node (Tier 2) | 80–86% | 5–8 | ✓ |
| qwen3-32b · qwen3-30b-a3b (open) | Single GPU — ~1× A100 80 GB (Tier 3) | 67–73% | 5–6 | ✗ |
| gemini-2.5-flash-lite (app lite) | Cloud API — Vertex (Tier 1) | 66% | 3 | ✗ |
| mistral-small-24b · gemma-3-27b (open) | Single GPU — ~1× A100 80 GB or a 128 GB Mac (Tier 3) | 52–64% | 2–5 | ✗ |

All 14 rows are complete single runs of n = 33 (no errors). Full descent: sonnet-4.6/5 96 · opus/haiku 93 · gemini-3.5-flash 92 · **qwen3-8b 88** · gemini-2.5-flash / deepseek-v4-flash 86 · qwen3-next-80b 80 · qwen3-32b 73 · qwen3-30b-a3b 67 · gemini-2.5-flash-lite 66 · mistral-24b 64 · gemma-3-27b 52.

**Cost shape** (detailed in [self-hosting](self-hosting.qmd)): Tier 1 is **pay-per-token** — cheap for the flash/lite tiers, materially more for the frontier (opus-class ~2 orders of magnitude above flash-class per token). Tiers 2–3 are **amortised hardware, no per-call cost, data stays on-prem** — a one-time GPU rather than a running bill. The standout is that the ≥ 80% floor is *reachable at Tier 3 today*: `qwen3-8b` hits 88% on a single consumer GPU — the cheapest possible self-host — which is the sharpest version of the "self-hosting is viable now for this task" finding.

**Three reads for our decisions:**

1. **The frontier is saturated for this task.** Claude and gemini-3.5-flash cluster at 92–96%; the spread is within the small-sample noise. Well-posed stx numeric physics is *not* frontier-hard — which reframes "always want state-of-the-art" for this task class specifically.
2. **The app's own tiers descend sharply:** gemini 3.5-flash 92 → 2.5-flash 86 → 2.5-flash-lite 66. The lite tier misses ~1 in 3 — under-powered for physics correctness; **2.5-flash is the app's sweet spot** for this class, with 3.5-flash buying the last few points.
3. **Self-hosting is viable for this task today.** An 8B (qwen3-8b, 88%) and mid-size MoEs (~80%) clear the floor — a departmental GPU, or even a single card, could serve stx-physics correctness on-prem, keeping student data off cloud APIs entirely. This tightens the [self-host migration](self-hosting.qmd) estimate for T1 from "someday" to "now."
...
**Validity.** *Internal:* the calibration gate (top tier ≥ ~95% on the official keys) passes at 96%, so the top tier's answers are provisionally trustworthy as keys. *External:* across the open-weights, STX correlates with (re-verified) published GPQA-Diamond at **Pearson r ≈ 0.76** (n = 7) — genuine external validity; the eval tracks the same underlying capability as an established benchmark. But it does **not** substitute for measurement, and the reason is decision-critical: **`qwen3-8b` is a stark outlier** — the *lowest* GPQA of the open models (63.3) yet the *highest* STX (88%), because GPQA is graduate-level while STX is upper-secondary. STX sits ~15–25 points above GPQA (the task is easier), and the gap is widest for small models. So a GPQA-based shortlist would have **dismissed the single most decision-relevant model** — frontier-level accuracy on the cheapest self-host. Correlated enough to trust the method; divergent enough that the task-specific run is necessary. *(n = 7; `deepseek-v4-flash` uses DeepSeek-V3.2 GPQA as proxy; thinking/non-thinking mode confounds some published figures — treat r as directional.)*

**Limitations — read before citing.**

- **Provisional / unverified.** Single run, **n = 33**, no repeats. Small-sample noise is visible (e.g. qwen3-8b scoring above qwen3-32b — an ordering that shouldn't hold, and a marker of the noise floor). Treat bands, not exact ranks.
- **Judge-graded**, not yet human-audited. The methodology's human-calibration sample (~20 items graded by AR or a physics teacher) is the next step.
- **Figure / graph items excluded** pending expert (AR / JB) review — this is the *text-solvable* subset only, not the full paper.
...
*Snapshot 2026-07-15. Numbers will move as the item set grows, runs repeat, and the judge is human-calibrated.*
