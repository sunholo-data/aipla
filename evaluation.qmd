...
- **For tasks needing 80%+ capability:** UCPH-hosted Qwen 3.5 27B or Gemma 4 31B (Tier 3, single machine) is viable today.
- **For tasks needing 88–92% capability:** UCPH IT-hosted DeepSeek V4 Pro (Tier 2, GPU cluster) sits at 90.1% — viable today if the cluster exists.
- **For tasks needing 93%+ capability:** Cloud API is still the answer in 2026, but the gap narrows every release cycle.
- **For light tasks:** Tier 4 (on-device) is already operational for what it's designed for.

Llama is intentionally not in any of these lists — current open-source workloads are dominated by DeepSeek, Qwen, and Gemma generations per [OpenRouter usage rankings](https://openrouter.ai/rankings).
