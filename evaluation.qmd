...
The figure-dependent items above were excluded because a text-only model cannot see the graph. AILANG's runtime gained **image input** (`std/ai` v0.30.0), so the harness now attaches the problem's figure(s) and lets a **vision-capable** model read them directly. This is a **separate, smaller axis** — the 8 held graph-reading subquestions, scored only on models that accept images (Claude is absent: the account is usage-capped until 1 Aug). Same grader as the text panel at the wider graph-reading tolerance (0.10); the figure is sent as an image with the answer schema demanded in the prompt.

| Model | Graph-reading accuracy (mean ± std, 5 runs × 8 items) |
|---|---|
| gemini-3.5-flash (app top) | 98 ± 5 |
| gemini-2.5-flash (app mid) | 93 ± 6 |
| gpt-5-mini | 75 ± 0 |
| gemma-3-27b (open, self-host) | 60 ± 18 |
| mistral-small-3.2-24b (open, self-host) | 43 ± 10 |
| gemini-2.5-flash-lite (app lite) | 40 ± 9 |

**What it shows.** *Vision turns abstentions into scores.* Items that were pure "I can't see it" abstentions on the text panel are now solved — the frontier clears them cleanly (gemini-3.5-flash **98 ± 5**), and the app-tier descent repeats (3.5-flash → 2.5-flash → lite). **Self-hostable open-weights can read graphs**, just weaker and far noisier (gemma-3-27b 60 ± 18, with occasional hard errors; mistral-small-3.2 43 ± 10) — a locally-run model is not blind to figures, but figure-reading is where the frontier gap widens most.

**Read the bands, not the ranks — more so than above.** At **n = 8** one item is ± 12.5 pts, so even over 5 runs the std stay large (5–18) and only three groups actually separate: frontier Gemini (~93–98), gpt-5-mini (75 — oddly *zero*-variance, missing the same two items every run), and the floor (40–60, mutually overlapping). This is a **proof the capability is now measurable**, not a precise leaderboard; growing the graph-item set and an expert check of the figure-to-problem mapping (AR / JB) are the next steps.
