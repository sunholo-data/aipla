...
- **Tier 4 (on-device) currently lags by ~24–28 months** on hard reasoning — but the lag is partly self-imposed, not hardware-bound. Apple Intelligence and Gemini Nano deliberately cap parameter counts at ~3 B for battery and latency. Modern iPhones (15 Pro+) and Pixels have headroom for 7–14 B models with int4 quantisation; the constraint is increasingly software / product choice, not silicon. Expect on-device to compress faster than the projection suggests as Apple and Google revise their on-device model strategies.
