...
A short non-technical pass over the architecture before the implementation details. The diagrams below are what Jesper and Aswin will want to walk teachers through; the ADRs and the technical system view below them are the engineering detail underneath.
...
```{mermaid}
flowchart TB
    Teacher["👤 <b>Teacher</b><br/>Logs in with UCPH credentials"]

    subgraph Classes["Classes (a teacher has many)"]
        direction LR
        ClassA["Class 7B<br/>Physics A"]
        ClassB["Class 8A<br/>Physics A"]
    end

    subgraph Groups["Groups (anonymous; a class has many)"]
        direction LR
        Group1["bold-kazoo-87<br/>(3 students<br/>share a phone)"]
        Group2["ruby-petal-72<br/>(3 students<br/>share a phone)"]
        Group3["fluffy-goose-56<br/>..."]
    end

    subgraph Lessons["Lessons (a group has access to specific ones)"]
        direction LR
        L1["Boldkast<br/>projectile motion"]
        L2["Pendul<br/>simple harmonic motion"]
        L3["..."]
    end
