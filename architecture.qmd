...
| 015 | Unified chat UI — no separate teacher dashboard | Decided | M |
...
## ADR-015 — Unified chat UI: no separate teacher dashboard

**Context.** Teachers and students have different needs from AIPLA — teachers configure skills, review artefacts, monitor budgets and chat logs; students engage with tutors and consume artefacts. The reflex is to build a separate teacher dashboard alongside the student chat. The reflex would double the v1 surface area.

**Decision.** Use **the same chat UI** for both audiences. Role differentiation happens via the **skills exposed** to each role, not via separate routes. Teacher-role skills emit A2UI dashboard components or MCP App previews directly into the chat conversation — the conversation **is** the dashboard.

| Teacher task | Skill | Surface |
|---|---|---|
| See current class usage + budget | `class-status` | A2UI dashboard block |
| Generate a physics simulation | `physics-sim-builder` | MCP App preview |
| Review and publish pending artefacts | `review-artefacts` | A2UI list with inline preview + approve/reject |
| Create class / generate group IDs | `manage-class` | A2UI form |
| Search anonymised chat logs | `chat-log-search` | A2UI search results |
| Configure a problem-set helper bot | `problem-set-helper-config` | A2UI configuration form |

**Why this works:**

- **One UI surface to maintain.** No `/teacher` routes, no separate auth flow, no second deploy target.
- **Aligns with the [AI Protocol Platform template](#platform-foundation) philosophy** — everything is a skill; skills emit rich UI via A2UI / MCP Apps. Building a separate dashboard would go against the grain.
- **Auth-driven role filtering.** UCPH SSO sessions get teacher-role skills; anonymous group-ID sessions (per [ADR-001](#adr-001-student-identity-no-auth-anonymous-group-ids)) get student-role skills. The skill registry filters by role.
- **Discoverability** via a curated starter set of skills each role sees on open — effectively a personalised "home" for each audience without a custom page.

**Consequences.**

- Teacher experience is conversational, not page-based. For teachers who want a more traditional layout, a `dashboard-view` skill can return a multi-panel A2UI block — still inside the chat surface, visually richer.
- Mobile-friendly by default — same UI on phone or laptop.
...
Both [A2UI](https://a2ui.org/) and [MCP Apps](https://modelcontextprotocol.io/extensions/apps/overview) support **server-pushed updates to existing rendered components** — the teacher does not need to re-send a query to refresh a panel.

- **A2UI** binds data values to component paths. The agent emits a `dataModelUpdate` event (e.g., `/class/budget_used = 42`) and the client patches the affected component only ([Agentic UI architecture write-up](https://dev.to/tahmidbintaslim/agentic-ui-a2ui-ag-ui-build-uis-your-agent-can-update-in-real-time-274n))
- **MCP Apps** receive `ui/toolResult` messages over `postMessage` whenever the host's tool data changes, and can call `tools/call` back through the host bridge ([MCP Apps spec](https://blog.modelcontextprotocol.io/posts/2026-01-26-mcp-apps/))

Practical pattern for AIPLA: which skills benefit from live updates vs one-shot:

| Pattern | Skills | Why |
|---|---|---|
| **Live-update** (data-bound, server pushes patches) | `class-status`, `review-artefacts`, `chat-log-search` | Background state changes as students use the system — teacher shouldn't have to refresh |
| **One-shot** (generate → render → done) | `physics-sim-builder`, `illustration-builder`, `manage-class`, `misconception-pair` | Single user action completes a discrete task |

For live-update skills, the rendered component **persists in chat history at the position it was invoked** and continues receiving updates. The teacher can pin or scroll back to it; the data stays current.
