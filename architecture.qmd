...
**Order is rough priority** — the most defining decisions first (privacy posture, platform foundation, model strategy), then privacy mechanics, then infrastructure and application architecture, with the AILANG disclosure last.
...
- Routing layer ([ADR-008](#adr-008-model-abstraction-routing-layer)) handles all four tiers behind one interface. The AI Protocol Platform template already supports multi-provider routing; the on-device tier requires a thin client-side adapter (most platforms expose simple JS APIs).
