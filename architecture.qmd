...
| 3 — Server-local | **Self-hosted Ollama** running DeepSeek V4 / Qwen 3.5 / Gemma 4 (UCPH GPU server, or M's 128GB Mac during prototyping) | Per-task-class adoption — but the gap to cloud frontier has nearly closed: DeepSeek V4 Pro is at ~90% on GPQA Diamond. Server-local is viable for the bulk of AIPLA tasks *today*, not in some future migration. | Privacy-positive, zero API spend, no data crossing the institutional boundary. Adoption is **case-by-case**, not all-or-nothing. |
...
- **Cloud-agnosticism on the Claude path** is a strategic hedge: moving off GCP would only change auth endpoints, not providers.
- Cost mix: Haiku-class on cheap text, Gemini Flash on cheap multimodal, larger models only where the eval shows benefit. On-device tasks cost nothing.
- Migration trajectory: as the local-readiness fraction rises, tiers 3 and 4 grow and the cloud share shrinks.
- **On-device viability is per-device**: an iPad with Apple Intelligence has tier 4 available; a basic Chromebook doesn't. Router falls back tier 4 → 3 → 2 → 1 as needed.
