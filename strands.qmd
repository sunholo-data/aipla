...
|---|---|---|
| `physics-sim-builder` | Generates a self-contained HTML simulation from a topic prompt | MCP App (sandboxed iframe) |
| `illustration-builder` | Generates SVG/PNG illustrations for student materials | A2UI inline image |
| `misconception-pair` | Generates a *correct* illustration **and** a textbook-distractor wrong variant — pulls on the pedagogical pattern AR is already using to mine instructive AI failure modes | A2UI side-by-side |

**Bot configuration**

| Skill | What it does | Surface |
|---|---|---|
| `problem-set-helper-config` | Configures a student-facing tutor for a given problem set with hint structure | A2UI configuration form |
| `concept-dialogue-config` | Configures a Socratic conceptual-exploration tutor for a topic | A2UI configuration form |
| `lab-troubleshoot-config` | Configures a tutor that helps students debug experimental data | A2UI form + AILANG Parse for uploads |

**Dashboard / management**

| Skill | What it does | Surface |
|---|---|---|
| `class-status` | Current class usage, budget remaining, time-to-reset, active groups | A2UI dashboard block |
| `manage-class` | Create class, generate group IDs, list students-by-group | A2UI form |
| `review-artefacts` | Pending generated artefacts awaiting teacher approval before students see them | A2UI list with inline preview + approve/reject |
| `chat-log-search` | Search this teacher's anonymised class chat logs | A2UI search results |

### Student-facing (consumes teacher configurations)

| Skill | What it does |
|---|---|
| Problem-set hints | Chat-based step hints; never solves the problem fully |
| Conceptual exploration | Socratic dialogue on a topic the teacher pre-configured |
| Lab data troubleshooting | Photo / CSV / Tracker uploads + analysis |

### Stretch (post-Jutland, possibly post-summer)
...
**Goal.** Deploy purpose-configured chatbots ("pedagogical bots") that teachers can use with their stx physics classes — *and* support **teacher-generated interactive artefacts** (HTML simulations, illustrations, problem workbenches) rendered safely in chat. Pattern anchored on AR's existing GenAI trials, where AI is used as much for content generation as for tutoring.
