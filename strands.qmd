...
**Net v1 commitment.** Three student-facing physics skills are live (Boldkast, LED Planck, KineBot). Teacher UI is wired to a real backend. Chat logs land in BigQuery. v1.1 adds: student image upload, teacher activity creation and branching, proactive sim-reactive tutor, in-session student consent prompt, exit ticket, and group code persistence for a full school year.
...
- **Student image / document upload** — handwritten diagrams, experimental photos, draft notes. Tutor gives feedback grounded in the activity's curriculum concepts. Most-requested student feature in the 3 June check-in.
...
- **Bidirectional voice** — student speaks (browser Web Speech API) → tutor responds via TTS. New UX mode; research angle on whether voice changes Socratic interaction quality.
