...
|---|---|---|
| Problem-set hints | Chat-based step hints; never solves the problem fully | **✅ v0.1 shipped 2026-05-20** as `problem-set-hints` (Danish stx, Boldkast projectile motion) — live at [aipla-v01-frontend-wgwhd7mspa-lz.a.run.app](https://aipla-v01-frontend-wgwhd7mspa-lz.a.run.app/). + v1 expansion. |
...
| `physics-sandbox-tool` (Strand B) | Interactive 2D physics sandbox embedded as an MCP App — drag-and-drop objects, run simulations, query state via MCP tools. The "Algodoo-but-with-API" direction discussed in the 18 May meeting. Validated by AR's [LED Planck-constant lab](examples.qmd#a-procedural-virtual-lab-different-artefact-class) — a concrete instance of the form factor (movable equipment, click-terminal wiring, calibration, real lab procedure) AIPLA's stretch sandbox would generalise. | Build on [Matter.js](https://brm.io/matter-js/) or [Planck.js](https://piqnt.com/planck.js/) (MIT-licensed JS 2D physics). Algodoo itself is desktop-only with no external API; this stretch goal would give AIPLA an interactive scene-building surface that fits the MCP App architecture natively. |
