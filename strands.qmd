...
|---|---|---|---|
| `physics-sim-builder` | Generates a self-contained HTML simulation **plus** a paired Socratic-tutor system prompt referencing the simulation's specific UI. Output is consumed by a tutor-config skill (canonical: `problem-set-helper-config`) for the matching student-facing tutor. AR's preferred pattern. | MCP App (sim) + tutor-prompt feed into a config skill | Year-2 |
