...
**What the three screenshots show:**

- **Chat surface (`chat`):** Danish problem-set-hints tutor, Socratic scaffolding, AI-generated inline SVG diagrams.
...
- **Human tool-use cards** — moving a slider or changing a preset drops a card inline in chat (*"Justerede v₀ til 17.5 m/s ✓"*). The workbench narrates its state into the conversation rather than sitting as a silent sidebar — a conversation primitive beyond text + SVG + MCP-App panel.
- **Bidirectional loop:** student sets values on the sim → cards appear in chat → student asks *"can you see the values I set?"* → tutor confirms the exact values and uses them as the substance of its next Socratic prompt. The [ADR-015](architecture.qmd#adr-015-unified-multi-surface-ui-ai-directs-the-layout) multi-surface architecture doing real pedagogical work, not just rendering side-by-side.
- **Group-ID join:** anonymous, no PII (ADR-001); 30-day TTL friendly codes (`bold-kazoo-87`, `ruby-petal-72`); teachers hand them out.
- **Responsive:** desktop 50/50 split (screenshots 1–2) collapses to `Chat | Arbejde` tabs on phone (screenshot 3), matching the brief's *"one shared phone per three students"* — phone form factor without losing the multi-surface architecture.
...
The system prompt below was **built for the simpler paired-chatbot simulation** (not the richer one embedded above) and designed alongside it as a single unit — the canonical AIPLA pattern: the tutor's instructions name specific visual elements of its paired sim, so students know what to look at when it asks *"what do you notice about the blue arrow?"* It shows strict Socratic constraints, three teaching phases, hidden concepts to be discovered rather than stated, and polite off-topic redirects — the kind of configuration teachers will produce in AIPLA via `problem-set-helper-config` and `concept-dialogue-config`, through structured A2UI forms rather than raw text.

The original file calls Anthropic directly with a browser-entered API key; AIPLA replaces that with server-side central key management ([ADR-014](architecture.qmd#adr-014-per-group-per-class-budget-enforcement)) — same pattern, no key ever seen by teacher or student.
...
**[→ Open KineBot](kinebot.qmd)** — full description and interactive embed.
...
| Procedural virtual lab | [LED Planck-constant](led-planck.qmd) | `physics-lab-builder` | Mimic real experimental procedure with equipment and measurement workflow |
| Hybrid AI platform | [KineBot](kinebot.qmd) | `kinebot-kinematics-tutor` | Simulation + Socratic tutor + adaptive quiz in one artefact |
